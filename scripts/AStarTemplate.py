...
def aStar(start, goal, mapData):
    ...
    closedArray = reshapedMap[:]
    closedArray = numpy.subtract(closedArray,reshapedMap)
    openArray = reshapedMap[:]
    openArray = numpy.subtract(openArray, reshapedMap)
    closedpoints = set()
    finalpath = set()
    ...
    while (len(openset) != 0):  # while there are still nodes that have not been checked, continually run the algorithm
        ...
        print(len(openset))
        if (
                current.x == goalPose.position.x and current.y == goalPose.position.y):  # if the best possible path found leads to the goal, it is the best possible path that the robot could discover
        ...
        closedArray[current.x, current.y] = 100
        neighborSet = neighbor_nodes(current)
        for neighbor in neighborSet:  # re-evaluate each neighboring node

            if (checkIfListContainsPoint(closedpoints, neighbor) and neighbor not in openset and checkExistingNodes(
                    neighbor)):
                heapq.heappush(openset, neighbor)  # add neighbor to open set
                openArray[neighbor.x,neighbor.y] = 100
        ...
        return {'closedset': closedset, 'closedpoints': closedpoints, 'closedarray': closedArray, 'openarray': openArray,'openset': openset, 'finalpath': finalpath}
    print("failed")
    # return failure #if the program runs out of nodes to check before it finds the goal, then a solution does not exist


def aStarLoop(open_set, closed_set, closed_points, closed_Array, open_Array, final_path):
    while (len(open_set) != 0):  # while there are still nodes that have not been checked, continually run the algorithm
        ...
        if (
            ...
            return {'closedset': closed_set, 'closedpoints': closed_points, 'closedarray': closed_Array, 'openarray': open_Array,'openset': openset, 'finalpath': final_path}
        ...
        closed_Array[current.x, current.y] = 100
        neighborSet = neighbor_nodes(current)
        for neighbor in neighborSet:  # re-evaluate each neighboring node

            if (checkIfArrayContainsPoint(closed_Array, neighbor) and open_Array[neighbor.x, neighbor.y]<50 and checkExistingNodes(neighbor)):
                heapq.heappush(openset, neighbor)  # add neighbor to open set
                open_Array[neighbor.x, neighbor.y] = 100
        ...
        #for node in closed_set:
            #closedsetPoints.cells.append(addScaledPoint(node.x, node.y))
        return {'closedset': closed_set, 'closedpoints': closed_points, 'closedarray': closed_Array, 'openarray': open_Array,
                    'openset': open_set, 'finalpath': final_path}
...
def checkIfArrayContainsPoint(list, point):
    if (list[point.x, point.y] >=50):
        return False
    else:
        return True
...
def heuristic_cost_estimate(startpose, goalpose):
    return kDistance * distance_calculation(startpose, goalpose) + kTurn * (
...
def gValueFunction(currentNode, i, j):
    if (currentNode.isStart):
        ...
        return kTurn * (
        math.atan2(currentNode.y - currentNode.parent.y, currentNode.x - currentNode.parent.x) - math.atan2(j,
